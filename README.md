[permutation-combination-calculator](https://dirkarnez.github.io/permutation-combination-calculator)
===================================================================================================
Electronic calculators cannot list permutations / combinations it calculates

### API
- [itertools — Functions creating iterators for efficient looping — Python 3.14.0 documentation](https://docs.python.org/3/library/itertools.html)

### Online math-based calculators
- [Permutations Calculator nPr](https://www.calculatorsoup.com/calculators/discretemathematics/permutations.php)
